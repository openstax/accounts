...
feature 'User logs in', js: true do
  ...
  scenario 'with an email address linked to several user accounts' do
    with_forgery_protection do
      ...
      complete_login_username_or_email_screen 'user@example.com'
      expect_sign_in_page
      expect(page).to have_content(t("legacy.sessions.start.multiple_users.content_html").sub('<br/>', '').sub(' %{link}.', ''))
      screenshot!
  ...
  scenario 'views login help' do
    ...
    visit '/'
    expect_sign_in_page
    expect(page).to have_content(t :"legacy.sessions.start.having_trouble")
    expect(page).to have_no_content(t :"legacy.sessions.start.help")
    expect(page).to have_no_link(t :"legacy.sessions.start.knowledge_base")

    click_link t :"legacy.sessions.start.having_trouble"
    expect(page).to have_content(t :"legacy.sessions.start.help")
    expect(page).to have_link(t :"legacy.sessions.start.knowledge_base", target: "_blank")
    screenshot!
  end
