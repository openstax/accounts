RSpec.shared_examples "add_reset_password_shared_examples" do |parameter|
  ...
  scenario 'using a link with a valid code' do
    visit start_path(type: type, token: @login_token)
    expect(page).to have_no_missing_translations
    expect(page.first('#set_password_password_confirmation')["placeholder"]).to eq t :"legacy.identities.set.confirm_password"
    expect_page(type: type)
  end
  ...
  scenario 'with a blank password' do
    visit start_path(type: type, token: @login_token)
    expect_page(type: type)
    click_button (t :"legacy.identities.#{type}.submit")
  ...
  scenario 'password is too short' do
    ...
    expect(page).to have_no_missing_translations
    expect_page(type: type)
    fill_in (t :"legacy.identities.set.password"), with: 'pass'
    fill_in (t :"legacy.identities.set.confirm_password"), with: 'pass'
    click_button (t :"legacy.identities.#{type}.submit")
  ...
  scenario 'successful' do
    visit start_path(type: type, token: @login_token)
    expect(page).to have_no_missing_translations
    fill_in (t :"legacy.identities.set.password"), with: '1234abcd'
    fill_in (t :"legacy.identities.set.confirm_password"), with: '1234abcd'
    click_button (t :"legacy.identities.#{type}.submit")
    expect(page).to have_content(t :"legacy.identities.#{type}_success.message")
    click_button (t :"legacy.identities.#{type}_success.continue")
  ...
  scenario 'cancels reset' do
    ...
    fill_in (t :"legacy.identities.set.confirm_password"), with: '1234abcd'
    fill_in (t :"legacy.identities.set.confirm_password"), with: '1234abcd'
    click_link (t :"legacy.identities.set.cancel")
    expect_profile_page
