...
describe SearchApplicationUsers do
  ...
  it "should not return results if application is nil" do
    outcome = SearchApplicationUsers.call(nil, 'username:jstra').outputs.users
    expect(outcome).to eq nil
  end

  it "should match based on username" do
    outcome = SearchApplicationUsers.call(application, 'username:jstra').outputs.users
    expect(outcome).to eq [user_1]
  end

  it "should ignore leading wildcards on username searches" do
    outcome = SearchApplicationUsers.call(application, 'username:%rav').outputs.users.to_a
    expect(outcome).to eq []
  end

  it "should match based on one first name" do
    outcome = SearchApplicationUsers.call(application, 'first_name:"John"').outputs.users.to_a
    expect(outcome).to eq [user_3, user_1]
  end

  it "should match based on one full name" do
    outcome = SearchApplicationUsers.call(application, 'full_name:"Mary Mighty"').outputs.users.to_a
  ...
  it "should match based on an exact email address" do
    ...
    outcome = SearchApplicationUsers.call(application, "email:#{email}").outputs.users.to_a
    expect(outcome).to eq [user_1]
  end
  ...
  it "should return all results if the query is empty" do
    outcome = SearchApplicationUsers.call(application, "").outputs.users.to_a
    expect(outcome).to eq [user_3, user_1, user_2]
  end

  it "should match any fields when no prefix given" do
    outcome = SearchApplicationUsers.call(application, "jst").outputs.users.to_a
    expect(outcome).to eq [user_3, user_1]
  end

  it "should match any fields when no prefix given and intersect when prefix given" do
    outcome = SearchApplicationUsers.call(application, "jst username:jst").outputs.users.to_a
    expect(outcome).to eq [user_3, user_1]
  end

  it "shouldn't allow users to add their own wildcards" do
    outcome = SearchApplicationUsers.call(application, "username:'%ar'").outputs.users.to_a
    expect(outcome).to eq []
  end

  it "should gather space-separated unprefixed search terms" do
    outcome = SearchApplicationUsers.call(application, "john mighty").outputs.users.to_a
    expect(outcome).to eq [user_3, user_1, user_2]
  end
  ...
  context "pagination and sorting" do
    ...
    it "should return the first page of values by default in default order" do
      outcome = SearchApplicationUsers.call(application, "username:billy").outputs.users.to_a
    ...
    it "should return the 2nd page when requested" do
      outcome = SearchApplicationUsers.call(application, "username:billy", page: 1).outputs.users.to_a
      expect(outcome.length).to eq 20
    ...
    it "should return the incomplete 3rd page when requested" do
      outcome = SearchApplicationUsers.call(application, "username:billy", page: 2).outputs.users.to_a
  ...
  context "sorting" do
    ...
    it "should allow sort by multiple fields in different directions" do
      outcome = SearchApplicationUsers.call(application, "username:foo", order_by: "first_name, last_name DESC").outputs.users.to_a
      expect(outcome).to eq [bob_jones, bob_brown, tim_jones]

      outcome = SearchApplicationUsers.call(application, "username:foo", order_by: "first_name, last_name ASC").outputs.users.to_a
