...
describe GroupNesting do
  ...
  let!(:group_nesting_1) { FactoryBot.build(:group_nesting) }
  let!(:group_nesting_2) { FactoryBot.build(:group_nesting) }
  ...
  context 'validation' do
    ...
    it 'cannot nest groups in loops' do
      gn = FactoryBot.build(:group_nesting, container_group: group_1, member_group: group_1)
      ...
      gn = FactoryBot.build(:group_nesting, container_group: group_2, member_group: group_1)
      expect(gn).not_to be_valid
      expect(gn.errors.messages[:base]).to eq(["would create a loop"])

      FactoryBot.create(:group_nesting, container_group: group_2, member_group: group_3)
      group_1.reload
      group_2.reload
      group_3.reload

      gn = FactoryBot.build(:group_nesting, container_group: group_3, member_group: group_1)
      expect(gn).not_to be_valid
      expect(gn.errors.messages[:base]).to eq(["would create a loop"])
    end
  end
