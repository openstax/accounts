...
describe Authentication do
  ...
  context "when an authentication exists" do
    it "is returned by by_provider_and_uid" do
      value = Authentication.by_provider_and_uid(authentication.provider, authentication.uid)
      expect(value).to eq authentication
    end

    it "is returned by by_provider_and_uid!" do
      value = nil
      expect{value = Authentication.by_provider_and_uid!(authentication.provider, authentication.uid)}
  ...
  context "when an authentication doesn't exist" do
    it "returns nil from by_provider_and_uid" do
      expect(Authentication.by_provider_and_uid("foo", "bar")).to be_nil
    end

    it "is created by by_provider_and_uid!" do
      provider = SecureRandom.hex(4)
      value = nil
      expect{value = Authentication.by_provider_and_uid!(provider, "42")}
