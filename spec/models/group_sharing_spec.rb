...
describe GroupSharing do
  ...
  context 'validation' do
    ...
    it 'must have a valid shared_with' do
      ...
      group_sharing.shared_with = FactoryGirl.build(:doorkeeper_application)
      expect(group_sharing).not_to be_valid
      expect(group_sharing.errors.messages[:shared_with]).to(
        eq(["must be either a User or a Group"]))

      group_sharing.shared_with = FactoryGirl.build(:user)
      expect(group_sharing).to be_valid

      group_sharing.shared_with = FactoryGirl.build(:group)
      expect(group_sharing).to be_valid
    end
