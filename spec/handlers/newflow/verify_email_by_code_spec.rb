  ...
  describe VerifyEmailByCode, type: :handler, vcr: VCR_OPTS do
    ...
    before(:all) do
      ...
      context 'when instructor' do
        ...
        it 'verifies the email address found by the given code' do
          allow_any_instance_of(InstructorSignup::ActivateEducator).to receive(:exec).with(user: user)
          expect(email.verified).to be(false)
          handler_call
          email.reload
          expect(email.verified).to be(true)
        ...
        it 'calls InstructorSignup::ActivateEducator' do
          expect_any_instance_of(InstructorSignup::ActivateEducator).to receive(:exec).with(user: user)
          handler_call
        end
