...
describe FetchBookData, type: :lib, vcr: VCR_OPTS do
  describe '#subjects' do
    subject { described_class.new.subjects }

    context 'success' do
      let(:book_subjects) do
        ["Math", "Science", "Humanities", "Social Sciences", "Business", "Essentials", "College Success", "High School"]
      end

      example do
        expect(subject).to match(book_subjects)
      end
    end
  ...
  describe '#titles' do
    subject { described_class.new.titles }
    ...
    context 'success' do
      ...
      example do
        expect(subject).to match(titles)
      end
    end
