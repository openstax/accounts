...
**Step 4 promotes to `pending_faculty` when there is no SheerID outcome.** `CompleteProfile` advances any user still below the SheerID tier (`incomplete_signup`/`no_faculty_info`) to `pending_faculty` through the ladder, so an educator who skipped SheerID ("my school isn't supported", `is_sheerid_unviable`) or whose webhook never landed reaches the CX queue. This used to be a side effect of the lead push recomputing the status; when that computation was removed, nothing replaced it, and skip-SheerID educators shipped to Salesforce as `incomplete_signup` (fixed after v17.3.3). Users SheerID already ranked keep that outcome. `rake accounts:repair_profile_complete_faculty_status` (`DRY_RUN=true`-able) repairs the ones left behind: `profile_completed_at` set, still `incomplete_signup`, not a student.

Why: the SheerID webhook used to set `confirmed_faculty` and then enqueue `CreateOrUpdateSalesforceLead`, which recomputed the status from `is_profile_complete` and stamped `incomplete_signup` over it on both the User and the Lead; step 4 was the only repair, and a failed step-4 save (picklist drift, a 129-char `Title`) or an abandoned step 4 left the Lead stuck there, after which the Lead→Accounts sync demoted the confirmed instructor. **`CreateOrUpdateSalesforceLead` never computes a status now** -- it writes what is stored -- and both back-syncs (`UpdateUserLeadInfo`, `UpdateUserContactInfo`) go through the ladder.

The webhook (`Newflow::EducatorSignup::SheeridWebhook`) persists **every** delivery on `sheerid_verifications` (`current_step`, `error_ids`, `rejection_reasons`, `segment`, `last_response`, `webhook_received_at`, `webhook_count`), including `error` steps that used to return before a row existed; `SheeridVerification#faculty_status_for_step` is the one step→status mapping. The user is resolved by `sheerid_verification_id` first (error steps carry no `personInfo`), then by verified email. An `expiredVerification` with no user is an anonymous attempt aging out and stays silent; one that changes a real user's status is reported to Sentry. The admin user form shows the stored verification. `FV_Status__c` is a **restricted picklist** on Lead (its own value list) and Contact (global value set `FV_Status`): a new enum value must be added in Salesforce before the code that writes it deploys, or every lead save fails. Both enums are positional -- append only.

`CreateOrUpdateSalesforceLead` retries a save once without a field Salesforce rejected as an invalid restricted-picklist value, but only for the allowlisted profile fields (`subject_interest`, `adoption_status`, `expected_start_semester`, `who_chooses_books`, `position`) and only when *every* rejected field is allowlisted; `verification_status` failing must surface. `Title` is truncated to Salesforce's 128, and step 4 refuses a longer `other_role_name`.

**Nightly `SyncEducatorLeads`** (`cron:day`): pass 1 creates an `incomplete_signup` Lead for newflow educators who verified email ≥ 24 h ago (≤ 90 days), have no Lead or Contact, capped at 500/run and gated by `Settings::Salesforce.push_incomplete_signup_leads_enabled`; pass 2 runs `UpdateUserLeadInfo`, which is watermark-based (`leads_synced_through`, 15-minute overlap, paged by Id) and was previously not scheduled at all. Repair tasks, both `DRY_RUN=true`-able: `rake accounts:repair_faculty_status_from_sheerid` re-derives statuses from stored verifications and re-pushes; `rake accounts:repush_incomplete_signup_leads` re-pushes every Lead still at `incomplete_signup` whose user ranks higher.

**Profile nudge.** A user who got a SheerID outcome but never finished step 4 (`User#needs_profile_nudge?`) is redirected to step 4 **once** (login or `/i/profile`, stamped in `profile_nudge_redirected_at`) and sees a dismissible banner after that; `profile_completed_at` is stamped by `CompleteProfile` so the lag is measurable. `is_newflow` defaults to **false** in the schema, so factory users in specs need it set explicitly wherever a newflow-only predicate is under test.
...
**Nightly user-activity sync.** `PushUserActivityToSalesforce` (`app/routines/push_user_activity_to_salesforce.rb`) runs four bounded passes, each gated by its own feature flag and backed by its own partial index on `users`: (1) student link/create -- resolves or creates each student's `Student__c` by uuid and stamps `salesforce_student_id`; (2) student login refresh -- updates `Last_Account_Login_Date__c` straight from the stored `salesforce_student_id`, no SOQL lookup; (3) instructor Contact login refresh -- same idea via the existing `salesforce_contact_id`; (4) last-seen refresh -- updates `Last_Website_Visit__c` from `users.last_seen_at` on both populations (students via `salesforce_student_id`, instructors via `salesforce_contact_id`), again with no SOQL lookup. Passes 1-2 are gated on `push_students_enabled`, pass 3 on `push_contact_logins_enabled`, pass 4 on `push_last_seen_enabled`, with its student half *additionally* gated on `push_students_enabled` so that flag stays a complete kill switch for `Student__c` writes. Passes 2-4 **never create** a record -- a user without a linked record is skipped deliberately, so `Student__c`/`Contact` doesn't balloon with rows whose only content is a date. Writes batch through the Restforce Composite/Batch API at 25 records per call. Pass 4 writes `Last_Website_Visit__c` as a raw Salesforce API name directly in the Composite payload, the same way passes 2-3 write `Last_Account_Login_Date__c` -- this keeps the pass independent of the gem's attribute mapping.

A `*_pushed_at` column that is NULL means "never sent" -- and SQL's `last_signed_in_at > pushed_at` evaluates to NULL, not true, so a naive predicate silently excludes exactly the records a backfill exists to reach. Every refresh pass tests `pushed_at IS NULL OR ...` for this reason. Every refresh pass also stamps the value it actually sent (`last_signed_in_at` / `last_seen_at`) rather than `Time.current`: a login or visit landing between loading a batch and stamping it would otherwise sit under a newer watermark and never be pushed. Pass 1 is the exception and keeps `Time.current` -- its login date can be nil, and a nil stamp would re-select the user for linking forever. Pass 4 keeps a **separate** stamp per population (`salesforce_student_last_seen_pushed_at`, `salesforce_contact_last_seen_pushed_at`) rather than one shared column: a user can hold both links at once -- an educator who switches to the student role keeps the Contact their lead converted into, and nothing clears `salesforce_contact_id` on a role change -- so a shared stamp would let the student half permanently suppress the Contact half.
...
**Educator step 4 must not unlink a SheerID-matched school.** The step-4 school field only appears once a user leaves the SheerID path ("Verify another way", `is_sheerid_unviable`), and by then the webhook may already have linked their school. The field's autocomplete fills `school_id` only when a suggestion is picked, so a retyped name arrives with `school_id` blank -- and `CompleteProfile` used to read that as "edited pick, clear the link". The next `CreateOrUpdateSalesforceLead` then found no school and stamped the Find Me A Home Account onto the Lead's `School__c` (labelled "Organization"), with that Account's Abu Dhabi address, while `Company` (labelled "School") kept the real name via `most_accurate_school_name`. `CompleteProfile#school_for_typed_name` now keeps the current school when the typed name fuzzy-matches it, otherwise falls back to `School.match_self_reported`, and the placeholder is excluded from every branch. A picked `school_id` still wins; `self_reported_school` keeps the typed text unless a suggestion was picked.

Remote models (`Student`, `Book#osc_url`, `Contact#last_account_login_date`, `Contact#last_website_visit`, all from gem 11.0.0) live in the `openstax_salesforce` gem, not the app. The gem requires each remote model explicitly in its own `lib/openstax_salesforce.rb` -- it does not autoload them, and a model missing from that require list fails as a confusing "duplicate factory" error far from the real cause.

**Salesforce → Accounts contact sync.** `UpdateUserContactInfo` fetches Contacts modified since a watermark, `Settings::Salesforce.contacts_synced_through`, minus a 15-minute overlap for clock skew, and pages them by `Id` 2,000 at a time. Only after every page succeeds does it advance the watermark, to the run's *start* time, so a Contact modified mid-run isn't skipped. With no watermark (first run, or after clearing it in admin settings to force a resync) it falls back to `number_of_days_contacts_modified` (default 7). This replaced a fixed 7-day window: a 9/23/2026 bulk update of ~150k Contacts was re-fetched by every run for a week, taking runs from ~70s to over 2h. An unknown `faculty_verified` value or a `save!` validation failure is reported to Sentry and skipped rather than aborting the run -- with a watermark, one bad record would otherwise pin it forever. The contact sync (`cron:5-minutes`, running every 5 minutes) and school sync (`cron:5-past-half-hour`) sit on separate crontab lines on purpose: bit-deployment wraps each line in its own `flock --nonblock`, so a slow run silently skips every later run of the same line. The contact sync's check-in sends its own `MONITOR_CONFIG`, so change that constant along with the schedule. The Lead-side sync (`UpdateUserLeadInfo`, run by `SyncEducatorLeads` in `cron:day`) works the same way with its own watermark and check-in; both apply `FV_Status__c` through the faculty-status ladder.
