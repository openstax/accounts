...
**All three branches write only the school-describing fields**, and none of them routes through `CreateOrUpdateSalesforceLead`. That routine is a full profile sync: it recomputes `faculty_status` for every non-student, so a school edit alone would drop a `confirmed_faculty` instructor back into the CS verification queue, and when nothing resolves it stamps the `Find Me A Home` fallback onto `user.school` itself -- which `most_accurate_school_city`/`_country` then read, since unlike `most_accurate_school_name` they don't exclude the fallback, and whose nil->fallback churn on `school_id` makes `UpdateSelfReportedSchool`'s no-op-save guard never hold. The Lead branch instead writes `school`, `city`, `country`, `self_reported_school`, `account_id`/`school_id` and state (via `SalesforceLeadState`, shared with `CreateOrUpdateSalesforceLead`) onto the Lead it finds by the stored id alone: a lookup miss is nothing to do here, never a signal to clear `salesforce_lead_id`, which `UpdateExistingSalesforceLead` owns reconciling. A converted Lead is followed to its Contact -- `converted_contact_id` is stored as `salesforce_contact_id` and the Contact branch writes the school there, because `update_contact` never does.

The first two resolve an Account id — the School's `salesforce_id`, else the `Find Me A Home` fallback Account — and write it asymmetrically. `Student__c#School__c` only fills a blank (same never-overwrite rule as pass 1), so an unmatched school lands the student in the review bucket. A Contact's `AccountId` is overwritten by a resolved School, but the fallback fills only a **blank** `AccountId`: an unmatched name must never pull a Contact off an Account Customer Experience may have set. None of the three branches, including the Lead one, ever writes the fallback onto `user.school`.
