...
A Contact takes `Self_Reported_School__c`: the user's answer, with the resolved Account's id in parentheses when Accounts matched one, and the name alone when it didn't. That trailing id is what makes the drift between a Contact's stated school and its `AccountId` measurable. A converted Lead is followed to its Contact and handled the same way -- `converted_contact_id` is stored as `salesforce_contact_id`, then the Contact branch runs.
