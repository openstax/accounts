...
**Salesforce → Accounts contact sync.** `UpdateUserContactInfo` (`app/routines/update_user_contact_info.rb`) keeps a watermark, `Settings::Salesforce.contacts_synced_through`, instead of always re-querying a fixed trailing window: each run fetches Contacts modified since `watermark - 15.minutes` (the overlap covers clock skew between our servers and Salesforce's `LastModifiedDate`) and, only after every page succeeds, advances the watermark to the run's *start* time so a Contact modified mid-run isn't skipped by the next run. Before a watermark exists (or after ops clears it in admin settings to force a full resync), the routine falls back to `number_of_days_contacts_modified.days.ago`. Contacts are paged with keyset pagination on `Id` (`BATCH_SIZE` = 2000) rather than loaded all at once, so the routine's memory footprint stays flat regardless of how many Contacts changed. This exists because a fixed 7-day window meant a 9/23/2026 bulk update of ~150k Contacts was re-fetched by every run for a week straight, taking runs from ~70s to over 2 hours. An unknown `faculty_verified` value or a `save!` validation failure is isolated to that one user/Contact pair (reported to Sentry, counted, and skipped) rather than aborting the whole run -- with a watermark, an unrescued failure would pin the watermark on that same bad record forever. `UpdateUserContactInfo` and `UpdateSchoolSalesforceInfo` run on separate crontab lines (`cron:10-to-half-hour` and `cron:5-past-half-hour`) rather than sharing one: prod wraps each crontab line in its own non-blocking `flock` (bit-deployment ansible), so a slow contact sync would otherwise cause every later run of that same line -- including the school sync -- to exit immediately without doing anything.
