...
**New leads carry an environment stamp.** `CreateOrUpdateSalesforceLead` sets `Accounts_Environment__c` (from `Rails.application.secrets.environment_name`) and `Accounts_Test_User__c` (`user.is_test?`) only in the branch that builds a brand-new `Lead`, never on one found by stored id, UUID or email -- a lead found by email could be a real person's, and relabeling it would hide them from Customer Experience. Contacts are never stamped either, for the same reason `update_contact` writes only signup-profile fields. The gem SELECTs every field it maps on every Lead/Contact query, so a field must exist in Salesforce -- and the Run As user must have read access to it -- before the gem version that maps it is deployed, or every query fails.
