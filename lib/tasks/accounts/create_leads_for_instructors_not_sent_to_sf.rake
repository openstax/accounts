namespace :accounts do
  ...
  task :create_leads_for_instructors_not_sent_to_sf, [:day] => [:environment] do |t, args|
    ...
    users = User.where(salesforce_contact_id: nil, salesforce_lead_id: nil, role: :instructor,
state: :activated, faculty_status: :confirmed_faculty).or(User.where.not(sheerid_verification_id: nil))
    users.each { |user|
