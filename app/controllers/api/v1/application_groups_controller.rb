class Api::V1::ApplicationGroupsController < OpenStax::Api::V1::ApiController
  ...
  description <<-EOS
    ...
    * `application_groups` &ndash; Array containing info about the ApplicationGroups whose updates were read. The 'id' and 'read_updates' fields are mandatory. "read_updates" should contain the last value for "unread_updates" received by the app.
    ...
    `application_groups = {id: 13, read_updates: 1}` &ndash; will decrease unread_updates by 1, setting it to 1. The group will be sent again the next time you call `updates`, so you won't miss the updated information.
  EOS
  def updated
