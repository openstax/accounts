class InstructorSignupController < SignupController
  ...
  def instructor_change_signup_email_form
    @email = unverified_user.email_addresses.first.value
    @total_steps = 4
  end

  def instructor_change_signup_email
    handle_with(
      ChangeSignupEmail,
      user: unverified_user,
      success: lambda {
        redirect_to(instructor_email_verification_form_updated_email_path)
      },
      failure: lambda {
        @email = unverified_user.email_addresses.first.value
        render(:instructor_change_signup_email_form)
      }
    )
  end

  def instructor_email_verification_form
    @total_steps = 4
    @first_name = unverified_user.first_name
    @email = unverified_user.email_addresses.first.value
  end

  def instructor_email_verification_form_updated_email
    @total_steps = 4
    @email = unverified_user.email_addresses.first.value
  end

  def instructor_verify_email_by_pin
    handle_with(
      SignupVerifyEmail,
      ...
      success: lambda {
        ...
        redirect_to(instructor_sheerid_form_path)
      },
      failure: lambda {
        @total_steps = 4
        @first_name = unverified_user.first_name
        ...
        render(:instructor_email_verification_form)
      }
    )
  end

  def instructor_sheerid_form
  ...
  def instructor_complete_profile
    handle_with(
      SignupProfileInstructorFinal,
      user: current_user,
      success: lambda {
        user = @handler_result.outputs.user
        security_log(:user_profile_complete, { user: user })
        clear_incomplete_instructor

        if user.is_educator_pending_cs_verification?
          redirect_to(instructor_pending_cs_verification_path)
        else
      ...
      failure: lambda {
        ...
        if @handler_result.outputs.is_on_cs_form
          redirect_to(instructor_cs_verification_form_url, alert: "Please check your input and try again. Email address and School Name are required fields.")
        else
  ...
  def instructor_pending_cs_verification
