...
class FindOrCreateUnclaimedUser
  ...
  def exec(options)
    unless options[:email] || options[:username]
      fatal_error(code: :invalid_input, message: (I18n.t :"routines.fund_or_create_unclaimed_users.must_provide_email_or_usernmae"))
    end
  ...
  def create_user(options)
    ...
    if options[:password]
      ...
      identity.save!
      user.authentications.create!(   # TODO review this creation of authentication (otherwise only in SessionsCallback)
