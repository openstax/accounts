class AddAccountToSalesforce
  ...
  def call
    ...
    users.each do |user|
      ...
      sf_ox_account.role = user.role.to_s,
      sf_ox_account.contact_id = user&.salesforce_contact_id,
      sf_ox_account.lead_id = user&.salesforce_lead_id,
      sf_ox_account.signup_date = user.created_at.strftime("%Y-%m-%dT%H:%M:%S%z")
      sf_ox_account.environment = Rails.application.secrets.environment_name
