class UpdateSchoolSalesforceInfo
  ...
  def call
    ...
    School.where('NOT EXISTS (SELECT * FROM "users" WHERE "users"."school_id" = "schools"."id")') do |schools|
      salesforce_ids = schools.map(&:salesforce_id)
      existing_salesforce_ids = OpenStax::Salesforce::Remote::School.select(:id).where(id: salesforce_ids).map(&:id)
