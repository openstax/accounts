...
OxAccount.Api = {
  ...
  displayProfile() {
    return OxAccount.Host.setUrl("/profile");
  },

  displayLogin(url) {
    return OxAccount.Host.setUrl(`/remote/start_login?start=${url}`);
  },

  displayLogout(url) {
    return OxAccount.Host.setUrl(`/remote/start_logout?start=${url}`);
  },

  // onLogin is actually called by our login completion handler,
  // we just forward data onto listening parent
  onLogin(data) {
    return OxAccount.proxy.post({onLogin: data});
  }
