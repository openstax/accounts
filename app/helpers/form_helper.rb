module FormHelper
  class One
    def initialize(f:, header: nil, limit_to: nil, context:, params: nil, errors: nil, error_field_classes: "error")
      @f = f
      @header = header
      @limit_to = limit_to
      @context = context
    ...
    def get_errors_div(name:)
      ...
      @context.content_tag(:div, class: "errors invalid-message", role: "alert") do
        error_divs = field_errors.map do |field_error|
          @context.content_tag(:div, class: @error_field_classes) { field_error.translate.html_safe }
        end
